...
## Do these DERs have a significant FWER adjusted p-value?
table(fullRegions$significantFWER[is.na(fullRegions$overlap10) & width(fullRegions) >= 10])
```
...
## Do these DERs have a significant FWER adjusted p-value?
table(fullRegions$significantFWER[is.na(fullRegions$overlap20) & width(fullRegions) >= 20])
```
...
## Check by whether the DER has a FWER adjusted p-value < 0.05
addmargins(table('Overlaps a DE segment' = fullRegions$overlap10, 'FWER p-value < 0.05' = fullRegions$significantFWER))
```

Regardless of whether the DER p-value, we see that `r round(sum(!fullRegions$overlap10, na.rm = TRUE) / sum(!is.na(fullRegions$overlap10)) * 100, 2)` percent of the DERs overlapping at least one segment (min overlap 10bp), incorrectly overlap a segment set not to be DE. 
...
## Check by whether the DER has a FWER adjusted p-value < 0.05
addmargins(table('Overlaps a DE segment' = fullRegions$overlap20, 'FWER p-value < 0.05' = fullRegions$significantFWER))
```

Regardless of whether the DER p-value, we see that `r round(sum(!fullRegions$overlap20, na.rm = TRUE) / sum(!is.na(fullRegions$overlap20)) * 100, 2)` percent of the DERs overlapping at least one segment (min overlap 20bp), incorrectly overlap a segment set not to be DE. 

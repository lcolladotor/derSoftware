...
ff <- getF(fit,fit0, y)
fullRegionGR$sigBonf <- factor(p.adjust(ff$f_pval,"bonf") < 0.05, levels = c('TRUE', 'FALSE'))

## Find overlaps with DE regions
ctov <- countOverlaps(tx, fullRegionGR)

## Check result with sig regs
bonf <-  fullRegionGR[fullRegionGR$sigBonf == 'TRUE']
min.ov <- min(min(width(bonf)), min(width(tx)))
ctov.bonf <- countOverlaps(tx, bonf, minoverlap =  min.ov)

## Use appropriate set
ctov.use <- if (identical(ctov > 0, ctov.bonf > 0)) ctov else ctov.bonf
```
...
The results are `r ifelse(identical(ctov > 0, ctov.bonf > 0), 'the same', 'not the same')` using a minimum overlap of `r min.ov` bp between transcripts and candidate regions with a Bonferroni adjusted p-value < 0.05. Thus, we will use `r ifelse(identical(ctov > 0, ctov.bonf > 0), 'all DE regions', 'only the DE regions with Bonferroni adjusted p-value < 0.05')`.


```{r 'stricter', bootstrap.show.code = FALSE}
addmargins(table('DE status' = chosen$DE, 'Overlaps DE region (sig Bonf.)' = ctov.bonf > 0))
```

At a finer level, there is a difference in the number of exons per transcript overlapping all candidate regions vs the regions with Bonferroni adjusted p-value < 0.05.

```{r 'detaildiff', bootstrap.show.code = FALSE}
## Verify things are working properly
# table(countOverlaps(tx, fullRegionGR, minoverlap = min.ov) - ctov.bonf)

## Difference in overlaps found
table(ctov - ctov.bonf)
```
...
Next we can evaluate the simulation by classifying the exonic segments as whether they should be DE or not. Then, we can find if the DE regions overlap such segments and viceversa. We would expect that the DE regions with a Bonferroni adjusted p-value < 0.05 would only overlap segments that were set to be DE.
...
We can check the if the exonic segments overlap one or more DE region similarly to what we did earlier at the transcript and gene level. The results change depending on whether only the DE regions with significant Bonferroni adjusted p-value or all of the DE regions are used.


```{r, 'segsvsders', bootstrap.show.code = FALSE}
addmargins(table('DE status' = segs$DE, 'Overlaps DE region (sig Bonf)' = countOverlaps(segs, bonf) > 0))

addmargins(table('DE status' = segs$DE, 'Overlaps DE region' = countOverlaps(segs, fullRegionGR) > 0))
```

Using the DE regions with significant Bonferroni adjusted p-values, there are `r sum(segs$DE & !countOverlaps(segs, bonf) > 0)` false negative cases. From the exploration shown below, half of them seem short. Most of the false negative segments correspond to genes from the _oneDE_ scenario. Thus revealing that the complexity of that scenario makes it challenging to identify significant DE regions.


```{r 'seg.fn'}
## Explore false negative segments using DE regions with sig Bonf
seg.fn <- which(segs$DE & !countOverlaps(segs, bonf) > 0)
...
## Do these DE regions have a significant Bonferroni adjusted p-value?
table(fullRegionGR$sigBonf[is.na(fullRegionGR$overlap)])
```
...
## Do these DE regions have a significant Bonferroni adjusted p-value?
table(fullRegionGR$sigBonf[is.na(fullRegionGR$overlap10) & width(fullRegionGR) >= 10])
```
...
## Do these DE regions have a significant Bonferroni adjusted p-value?
table(fullRegionGR$sigBonf[is.na(fullRegionGR$overlap20) & width(fullRegionGR) >= 20])
```
...
## Check by whether the DE region has a Bonferroni adjusted p-value < 0.05
addmargins(table('Overlaps a DE segment' = fullRegionGR$overlap, 'Bonf. p-value < 0.05' = fullRegionGR$sigBonf))
```
...
## Check by whether the DE region has a Bonferroni adjusted p-value < 0.05
addmargins(table('Overlaps a DE segment' = fullRegionGR$overlap10, 'Bonf. p-value < 0.05' = fullRegionGR$sigBonf))
```
...
## Check by whether the DE region has a Bonferroni adjusted p-value < 0.05
addmargins(table('Overlaps a DE segment' = fullRegionGR$overlap20, 'Bonf. p-value < 0.05' = fullRegionGR$sigBonf))
```
...
`r ifelse(table(fullRegionGR$overlap, fullRegionGR$sigBonf)[1,1] == 0, 'None of the DE regionss with a Bonferroni adjusted p-value < 0.05 overlaps a segment set not to be DE, which is what we would expect.', 'We were not expecting to see DE regions with a Bonferroni adjusted p-value < 0.05 overlaping a segment set no to be DE.')`

...
## Define a second incomplete gene set where 10% of the transcripts are 
## dropped at random, regardless of the simulation scenario
tx_rand <- tx[unique(chosen$gene_id)]
tx_rand_list <- unlist(tx_rand)
set.seed(20150406)
tx_rand_chosen <- tx_rand_list[sample(seq_len(length(tx_rand_list)), size = round(length(tx_rand_list) * 0.9))]

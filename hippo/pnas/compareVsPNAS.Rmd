...
regPlot <- function(region, title) {
    ...
    symbol <- as.character(mcols(annoReg$annotationList[[1]])$symbol)
...
sapply(sortWidth(fullRegs20[countOverlaps(fullRegs20, both[both$sig], minoverlap = 20L) > 0 & fullRegs20$sigFWER])[1:5], function(reg) {
...
sapply(sortWidth(both[width(both) >= 20 &both$sig & countOverlaps(both, fullRegions[fullRegions$sigFWER], minoverlap = 20L) > 0])[1:5], function(reg) {
    regPlot(reg, 'Exon query: DE agreement')
})

sapply(sortWidth(both[width(both) >= 20 & !both$sig & countOverlaps(both, fullRegions[fullRegions$sigFWER], minoverlap = 20L) == 0])[2:6], function(reg) {

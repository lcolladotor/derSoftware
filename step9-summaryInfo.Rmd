...
reads <- system(paste0('grep ', file.path(rootdir, opt$short, 'CoverageInfo', 'logs'), '/fullCov-*.e* | grep -v "that 0 percent"'), intern=TRUE)
...
```{r "exampleRegions", message=FALSE, fig.width=20, fig.height=10, dev="CairoPNG"}
...
regionClusterPlot <- function(idx, tUse="fwer") {
	...
	## Save as pdf
	pdf(file= file=file.path(resdir, paste0("exampleRegion", idx, ".pdf")), width=20, height=10)
	print(p)
	dev.off()
	rm(p.ideo, covInfo)

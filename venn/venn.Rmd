...
This report creates the CSV files with the candidate differentially expressed regions (DERs) found using `derfinder` `r citep(bib[["derfinder"]])` on three data sets. It also has Venn diagrams illustrating the overlap with known annotation. 
...
1. __qvalues__ The p-value adjusted to control the FDR by using the `qvalue` `r citep(bib[["qvalue"]], "Dabney et al")` function from the package with the same name.
1. __significantQval__ Whether the q-value is less than 0.10.
1. __name__ This and the following fields are computed using `annotateNearest()` from the `bumphunter` `r citep(bib[["bumphunter"]], "Irizarry et al")` package. They were calculated using the UCSC hg19 annotation. __name__ refers to the nearest gene.

...
Table of results and venn diagrams
==================================
...
This report creates the CSV files with the candidate differentially expressed regions (DERs) found using `derfinder` `r mycitep(bib[["derfinder"]])` on three data sets. It also has Venn diagrams illustrating the overlap with known annotation. 
...
for(exp in exps) {
    ...
    # read.csv("supplementaryFile4.csv")
    # read.csv("supplementaryFile5.csv")
}
...
The following Venn diagrams show how many candidate differentially expressed regions (DERs) overlap with an exon, intron, or intragenic regions of the UCSC hg19 knownGene annotation. By default, a minimum overlap of 20 base pairs is required to say that a candidate DER overlaps any of feature. The annotation overlap was done using the `mergeResults()` function from `derfinder` `r mycitep(bib[["derfinder"]])` while the Venn diagrams were made using `limma` `r mycitep(bib[["limma"]])`.

Three Venn diagrams are shown per data set. The first one uses all the candidate DERs, the second one uses only the candidate DERs that had a significant q-value (by default less than 0.10), and the third uses only the candidate DERs that had a FWER adjusted p-value less than 0.05. 
...
The supplementary files 1 through 5 files contain the information for the candidate DERs found in [CSV](http://en.wikipedia.org/wiki/Comma-separated_values) format. The CSV files contain the following columns:
...
1. __qvalues__ The p-value adjusted to control the FDR by using the `qvalue` `r mycitep(bib[["qvalue"]], "Dabney et al")` function from the package with the same name.
1. __significantQval__ Whether the q-value is less than 0.10.
1. __name__ This and the following fields are computed using `annotateNearest()` from the `bumphunter` `r mycitep(bib[["bumphunter"]], "Irizarry et al")` package. They were calculated using the UCSC hg19 annotation. __name__ refers to the nearest gene.
